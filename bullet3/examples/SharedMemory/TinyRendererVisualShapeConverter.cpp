	...
	class btCollisionObject* colObj, int bodyUniqueId)
{
	...
	if (linkPtr)
	{
		...
		for (int v1=0; v1<cnt; v1++)
		{
			...
			float rgbaColor[4] = {static_cast<float>(color[0]),static_cast<float>(color[1]),static_cast<float>(color[2]),static_cast<float>(color[3])};
...
void TinyRendererVisualShapeConverter::changeRGBAColor(int bodyUniqueId, int linkIndex, const double rgbaColor[4])
{
	...
	for (int i=0;i<m_data->m_swRenderInstances.size();i++)
	{
		...
		if (ptrptr && *ptrptr)
		{
			float rgba[4] = {static_cast<float>(rgbaColor[0]), static_cast<float>(rgbaColor[1]), static_cast<float>(rgbaColor[2]), static_cast<float>(rgbaColor[3])};
